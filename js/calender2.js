...
if( queries.dep_cd == "" || queries.dep_cd == undefined 
	...
	document.write("ご希望の日時を選択してください。<BR>");
...
function make_cal( queries, moveBase, move ) {
	...
	var calText = '';
	if( moveBase == 0 ) {
		calText = calText + '<input type="button" id="" value="前週" style="width:70px;height:30px" disabled>'
	} else {
		calText = calText + '<input type="button" id="" value="前週" style="width:70px;height:30px" onclick="refreshPage(' + moveBase + ', -1);">'
	}
	calText = calText + '<input type="button" id="" value="次週" style="width:70px;height:30px" onclick="refreshPage(' + moveBase + ', 1);">'
	...
	// 凡例テキスト表示
	calText = calText + '※　○：予約可能、×：予約不可、-：営業時間外<br>';
	
	// 予約枠が選択されていたら「予約」ボタンを表示する
	if( selId != '' ) {
		var text = '\'' + selId  + '-' + queries.dep_cd + '-' + queries.ptId + '-' + queries.btday + '\'';
		calText = calText + '<div class="confirm"><a href="#" class="btn-box" id="fetchLGSbtn" onclick="getReservation(' + text + ');">予約</a></div>';
		calText = calText + '<div class="flex"><div id="LGSlink" class="center">暗号化されたセキュアなリンクを生成します。</div></div>';
	}
...
async function fetchLinkData(bodyText) {
	...
	var revTime = LGSyear + "年" + LGSmonth + "月" + LGSday + "日"+" " + LGShour + "時" + LGSmin + "分";
	const LGSlink = document.querySelector('#LGSlink');
	LGSlink.innerHTML = ("<h2>" + "<b>" + dispRevText[0] + "</b>："+LGSsubject+"<br>"+"<b>予約日時</b>："+ revTime +"<br><br>"+"ご予約の日時になりましたら、"+"<br>"+"以下のリンクをクリックし、"+"<br>"+"オンライン" + dispRevText[1] + "にご入室ください。"+"<br><br>"+'<a href="'+LGSguest+links.guest+'" target="_blank" rel="noopener noreferrer" class="participantlink">【オンライン' + dispRevText[1] + "】" +'</a>'+"<br><br>"+'<div class="copy_btn" data-clipboard-text="'+LGSguest+links.guest+'">ゲスト用リンク</div>'+"<br>"+'<div class="copy_btn" data-clipboard-text="'+links2.link+'">ゲスト用リンク(短縮)</div>'+"<br>"+'<div class="copy_btn" data-clipboard-text="'+LGSguest+links.host+'">' + dispRevText[2] + '用リンク（ゲスト）</div>'+"<br>"+'<div class="copy_btn" data-clipboard-text="'+LGShost+links.host+'">' + dispRevText[2] + '用リンク（ユーザー）</div>'+"<br>"+'<a href="'+LGShost+links.host+'" target="_blank" rel="noopener noreferrer" class="participantlink_hide">' + dispRevText[2] + '</a>');
	...
	var raw = JSON.stringify({
		...
		"attachments": [
		{
			...
			"content": {
				...
				{
					"type": "TextBlock",
					"text": "時間になりましたら「オンライン" + meetingTitle + "開始」リンクから参加してください。",
					"wrap": true
				},
				{
					"type": "ColumnSet",
					...
					},
					{
						...
						{
							"type": "TextBlock",
							"text": "[オンライン" + meetingTitle + "開始](" + meetingLink + ")",
							"size": "Medium"
						}
						],
